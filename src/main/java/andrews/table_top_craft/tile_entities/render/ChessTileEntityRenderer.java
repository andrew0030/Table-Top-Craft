...
//							List<Integer> destinationCoordinates = new ArrayList<>(); TODO remove once done optimizing!
			...
			for (int rank = 0; rank < BoardUtils.NUM_TILES_PER_ROW; rank++) {
				for (int column = 0; column < BoardUtils.NUM_TILES_PER_ROW; column++) {
					...
					if (board.getTile(currentCoordinate).isTileOccupied()) {
						...
						int localPlayerTickCount = Minecraft.getInstance().player.tickCount;
...
//		final List<BasePiece> blackTakenPieces = new ArrayList<>();
		...
		Collections.sort(whiteTakenPieces, new Comparator<BasePiece>() {//TODO check if replacing the Collections.sort() with List.sort() works the same
			@Override
		...
		Collections.sort(blackTakenPieces, new Comparator<BasePiece>() {//TODO check if replacing the Collections.sort() with List.sort() works the same
			@Override
			public int compare(BasePiece piece1, BasePiece piece2) {
				return Ints.compare(piece2.getPieceValue(), piece1.getPieceValue());
			}
