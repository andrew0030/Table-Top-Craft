...
public class ClientPacketHandlerClass
{
    ...
    public static void handlePlayChessParticlesPacket(BlockPos pos, byte destCord, boolean isWhite, float xSpeed, float ySpeed, float zSpeed)
    {
        if(Minecraft.getInstance().level != null)
        {
            double xOffset = 0.0;
            ...
            if(Minecraft.getInstance().level.getBlockEntity(pos) instanceof ChessTileEntity blockEntity)
            {
                ...
                for(int i = 0; i < 10; i++)
                    Minecraft.getInstance().level.addParticle(TTCParticles.TINY_POOF, pos.getX() + xOffset, pos.getY() + (0.0625F * 13.5F), pos.getZ() + zOffset, 0, 0, 0);
                for(int i = 0; i < 14; i++)
